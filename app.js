...
/* ============================================================
   DRIVEEASE — app.js  (All Bugs Fixed v2)
   Fixes:
   - Vehicle images: upImgs now stores raw base64 strings (not objects)
   - Images display correctly in vehicle cards and detail modal
   - Insurance document viewable by vendor client in vehicle list
   - Insurance document viewable by admin in vehicle management
   - imgCounter removed (was causing stale-key issues), replaced with simple array push
   - removeImg now works by index
   - Form reset fully clears upImgs as []
...
// FIX: upImgs stores raw base64 strings, not objects. licFront/licBack/insDoc store base64.
...
/* FIX: Safe image src — returns the usable src string from whatever format is stored */
function safeImgSrc(img) {
  if (!img) return '';
  if (typeof img === 'string') return img;
  // Legacy object format {key, data} — extract .data
  if (typeof img === 'object' && img.data) return img.data;
  return '';
}

/* FIX: Get first valid image src from a vehicle's images array */
function getFirstImg(v) {
  var fallback = 'https://images.unsplash.com/photo-1492144534655-ae79c964c9d7?w=600&h=400&fit=crop';
  if (!v || !v.images || !v.images.length) return fallback;
  var src = safeImgSrc(v.images[0]);
  return src || fallback;
}
...
function signOut() {
  CU = null; CR = null;
  licFront = null; licBack = null; insDoc = null; upImgs = [];
  Store.del('session');
...
function vehicleCardHtml(v, isTrending) {
  // FIX: use safeImgSrc to handle both string URLs and legacy {key,data} objects
  var img = getFirstImg(v);
...
function openDetail(id) {
  ...
  if (!v) return;
  // FIX: normalize all images through safeImgSrc
  var rawImgs = (v.images && v.images.length) ? v.images : [];
  var imgs = rawImgs.map(safeImgSrc).filter(Boolean);
  if (!imgs.length) imgs = ['https://images.unsplash.com/photo-1492144534655-ae79c964c9d7?w=600&h=400&fit=crop'];

  var isUser = CU && CR === 'user';
  var isAdmin = CU && CR === 'admin';
  ...
  // FIX: Show insurance button for admin and user in detail modal
  var insBtn = '';
  if (v.insuranceDoc) {
    if (isAdmin) {
      insBtn = '<button class="btn-view-ins mt-2" onclick="viewInsuranceDoc(\'' + v.id + '\')"><i class="fas fa-file-shield"></i> View Insurance Document</button>';
    } else {
      insBtn = '<span class="ins-verified-tag"><i class="fas fa-shield-alt"></i> Insurance Verified</span>';
    }
  }

  var bookBtn = isUser
    ? '<button class="btn btn-de-primary w-100 mt-3" onclick="modalHide(\'vDetailModal\');setTimeout(function(){openBooking(\'' + v.id + '\')},300)"><i class="fas fa-calendar-check"></i> Book This Vehicle</button>'
    : '<button class="btn btn-de-primary w-100 mt-3" onclick="modalHide(\'vDetailModal\');setTimeout(openSignIn,300)"><i class="fas fa-sign-in-alt"></i> Sign In to Book</button>';
  ...
  $('#vDetailContent').html(
    '<div style="padding-right:30px">' +
      ...
      '<p style="font-size:.82rem;color:var(--text3);margin-bottom:10px"><i class="fas fa-store" style="color:var(--accent)"></i> ' + v.vendorName + '</p>' +
      insBtn +
      bookBtn +
    '</div>'
  );
...
/* FIX: View insurance document — creates a temporary modal */
function viewInsuranceDoc(vehicleId) {
  var vs = Store.get('vehicles') || [];
  var v = vs.find(function(x) { return x.id === vehicleId; });
  if (!v || !v.insuranceDoc) { showToast('No insurance document available', 'inf'); return; }
  var d = document.createElement('div');
  d.className = 'modal fade';
  var isImg = v.insuranceDoc.indexOf('data:image') === 0;
  d.innerHTML =
    '<div class="modal-dialog modal-dialog-centered modal-lg"><div class="modal-content de-modal">' +
      ...
      '<div class="mhdr"><h2><span class="accent">Insurance</span> Document</h2><p>' + v.name + ' — ' + v.vendorName + '</p></div>' +
      (isImg
        ? '<img src="' + v.insuranceDoc + '" style="width:100%;border-radius:12px;border:1px solid var(--border)"/>'
        : '<div style="display:flex;flex-direction:column;align-items:center;gap:14px;padding:30px"><i class="fas fa-file-pdf" style="font-size:4rem;color:var(--accent)"></i><p style="color:var(--text2)">PDF Document Uploaded</p>' +
          '<a href="' + v.insuranceDoc + '" download="insurance.pdf" class="btn btn-de-primary"><i class="fas fa-download"></i> Download PDF</a></div>') +
    '</div></div>';
...
function loadAdminVehicles(filter) {
  ...
  var rows = vs.map(function(v) {
    // FIX: Admin can view insurance document
    var insCell = v.insurance
      ? '<span class="sbadge s-approved"><i class="fas fa-shield-alt"></i> Yes</span>' +
        (v.insuranceDoc ? ' <button class="btn-de-warn ms-1" style="font-size:.65rem;padding:3px 8px" onclick="viewInsuranceDoc(\'' + v.id + '\')"><i class="fas fa-eye"></i> View</button>' : '')
      : '<span class="sbadge s-rejected">No</span>';
    return '<tr>' +
...
function loadVendorVehicles() {
  ...
  var rows = sortByFn(vs, function(a, b) { return a.name.localeCompare(b.name); }).map(function(v) {
    // FIX: Vendor can view insurance doc they uploaded
    var insCell = v.insurance
      ? '<span class="sbadge s-approved"><i class="fas fa-shield-alt"></i> Yes</span>' +
        (v.insuranceDoc ? ' <button class="btn-de-warn ms-1" style="font-size:.65rem;padding:3px 8px" onclick="viewInsuranceDoc(\'' + v.id + '\')"><i class="fas fa-eye"></i> View</button>' : '')
      : '<span class="sbadge s-rejected">No</span>';
    // FIX: Show vehicle thumbnail in vendor list
    var thumbSrc = getFirstImg(v);
    return '<tr>' +
      '<td><div style="display:flex;align-items:center;gap:10px"><img src="' + thumbSrc + '" style="width:48px;height:38px;object-fit:cover;border-radius:7px;border:1px solid var(--border)" onerror="this.style.display=\'none\'"/><strong>' + v.name + '</strong></div></td>' +
...
function handleInsuranceUpload(input) {
  ...
  reader.onload = function(ev) {
    ...
    $('#insStat').html('<i class="fas fa-check" style="color:var(--success)"></i> Insurance uploaded ✓');
...
/*
  FIX (critical): upImgs is now a plain array of base64 strings.
  Previously it stored {key, data} objects which caused images to fail
  when passed as src to <img> tags.
  removeImg now removes by array index stored in the button data attribute.
*/
function handleVehicleImages(input) {
  ...
  toProcess.forEach(function(f) {
    ...
    reader.onload = function(ev) {
      var data = ev.target.result;
      if (!data) return;
      // FIX: push raw base64 string, not an object
      upImgs.push(data);
      var idx = upImgs.length - 1;
      $('#imgPrevs').append(
        '<div class="img-pitem" id="ip_' + idx + '">' +
          '<img src="' + data + '" alt="Vehicle photo"/>' +
          '<button class="img-rm" onclick="removeImg(' + idx + ',event)" type="button"><i class="fas fa-times"></i></button>' +
        '</div>'
      );
    };
...
/* FIX: removeImg by index — sets that slot to null and hides the DOM element */
function removeImg(idx, e) {
  if (e) e.stopPropagation();
  if (idx >= 0 && idx < upImgs.length) {
    upImgs[idx] = null; // null it out; filter on save
  }
  $('#ip_' + idx).remove();
}
...
function resetVehicleForm() {
  ...
  $('#vDesc').val('');
  $('#vType').val('sedan');
  // FIX: reset image state completely
  $('#imgPrevs').empty();
  upImgs = [];
  // FIX: reset insurance
  insDoc = null;
...
function addVehicle() {
  ...
  if (isNaN(price) || price <= 0) {
    showToast('Enter a valid daily rate', 'err');
    return;
  }
  if (isNaN(seats) || seats <= 0) {
    showToast('Enter a valid seat count', 'err');
    return;
  }

  // FIX: filter out any null slots from removeImg
  var validImgs = upImgs.filter(function(img) { return img !== null && img !== undefined; });
  ...
  var newVehicle = {
    ...
    vendorName: CU.name,
    status: 'pending',
    // FIX: store images as plain base64 strings array
    images: validImgs,
    // FIX: store insurance doc so it can be viewed later
    insuranceDoc: insDoc,
    insurance: true,
    trending: false,
    rating: null,
  ...
  showToast('Vehicle submitted! Awaiting admin approval ✓', 'ok');

  // Full reset after successful submission
  resetVehicleForm();

  // Switch to My Vehicles tab to show the added vehicle
  setTimeout(function() {
